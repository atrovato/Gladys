...
const awoxUtils = require('../../../../../services/awox/lib/mesh/awox.mesh.utils');

const generateRandomBytes = () => Buffer.from(randomBytes);
const checksum = (sessionKey, nonce, payload) => {
  if (checksumBytes.length > 0) {
    return Buffer.from(checksumBytes);
  }
  return awoxUtils.checksum(sessionKey, nonce, payload);
};

const { generatePairCommand, generateCommandPacket, decryptPacket, cryptPayload } = proxyquire(
  '../../../../../services/awox/lib/mesh/awox.mesh.commands',
  {
...
describe('awox.mesh.commands.generateCommandPacket', () => {
  ...
  it('Generate command', () => {
    ...
    const commandPacket = generateCommandPacket(peripheralUuid, sessionKey, { key, data });
...
describe('awox.mesh.utils.decryptPacket', () => {
  ...
  it('Decrypt packet', () => {
    ...
    const result = decryptPacket(peripheralUuid, sessionKey, commandPacket);
  ...
  it('Decrypt invalid packet', () => {
    ...
    try {
      decryptPacket(peripheralUuid, sessionKey, commandPacket);
      expect.fail();
    } catch (e) {
...
describe('awox.mesh.commands.cryptPayload', () => {
  ...
  it('Crypt Payload', () => {
    ...
    const commandPacket = cryptPayload(key, nonce, payload);
