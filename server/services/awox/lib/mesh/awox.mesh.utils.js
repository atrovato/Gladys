...
const { PARAMS, DEFAULTS } = require('./awox.mesh.constants');
 ...
 */
function generateRandomBytes(length = DEFAULTS.SESSION_LENGTH) {
...
function extractCredentials(device) {
  const userParam = device.params.find((p) => p.name === PARAMS.MESH_USER);
  if (!userParam) {
    throw new NoValuesFoundError(`AwoX Mesh: ${PARAMS.MESH_USER} is mandatory on device ${device.selector}`);
  }

  const passwordParam = device.params.find((p) => p.name === PARAMS.MESH_PASSWORD);
  if (!passwordParam) {
    throw new NoValuesFoundError(`AwoX Mesh: ${PARAMS.MESH_PASSWORD} is mandatory on device ${device.selector}`);
  }
...
/**
 * @description Determine device default credententials.
 * @param {Object} device - Device.
 * @param {boolean} remote - Remote device.
 * @returns {Object} Credentials.
 * @example
 * getDefaultCredentials(device);
 */
function getDefaultCredentials(device, remote) {
  let user = `${DEFAULTS.REMOTE_PREFIX}${device.external_id.substring(16).toUpperCase()}`;
